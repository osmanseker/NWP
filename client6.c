...
{
    //bericht
    const char *BerichtPlayer = (argc > 1)? argv [1]: "AmongUs>player6!>";
...
while(1)
{
    ...
    zmq_setsockopt (subscriber, ZMQ_SUBSCRIBE, "AmongUs>imposter6?>", 19);
    zmq_setsockopt (subscriber, ZMQ_SUBSCRIBE, "AmongUs>crewmember6?>", 21);
    memset(buffer,0,256);
    zmq_recv(subscriber, buffer, 256,0);
    ParsedString = parse(3, buffer);
    printf("%s\n", ParsedString);
    ...
    zmq_setsockopt(subscriber, ZMQ_UNSUBSCRIBE, "AmongUs>imposter6?>", 19);
    zmq_setsockopt(subscriber, ZMQ_UNSUBSCRIBE,"AmongUs>crewmember6?>", 21);
    ...
    for (int i = 0 ;i<players;i++)
    {
        ...
        k: zmq_setsockopt(subscriber, ZMQ_SUBSCRIBE, "AmongUs>kick6?>", 15);
        memset(buffer,0,256);
        zmq_recv(subscriber, buffer, 256,0);
        ParsedString = parse(3, buffer);
        printf("%s\n", ParsedString);
    ...
    zmq_setsockopt(subscriber, ZMQ_SUBSCRIBE, "AmongUs>kick6?>", 15);
    ...
    e: if((strcmp(ParsedString, "You have been kicked\n")) == 0)
    {
        ...
        zmq_setsockopt(subscriber, ZMQ_UNSUBSCRIBE, "AmongUs>kick6?>", 15 );
