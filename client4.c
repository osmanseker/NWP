...
while(1)
{
    ...
    zmq_setsockopt (subscriber, ZMQ_SUBSCRIBE, "AmongUs>imposter4?>", 19);
    zmq_setsockopt (subscriber, ZMQ_SUBSCRIBE, "AmongUs>crewmember4?>", 21);
    memset(buffer,0,256);
    zmq_recv(subscriber, buffer, 256,0);
    ParsedString = parse(3, buffer);
    printf("%s\n", ParsedString);
    ...
    zmq_setsockopt(subscriber, ZMQ_UNSUBSCRIBE, "AmongUs>imposter4?>", 19);
    zmq_setsockopt(subscriber, ZMQ_UNSUBSCRIBE,"AmongUs>crewmember4?>", 21);
    for (int i = 0 ;i<players;i++)
    {
        ...
        k: zmq_setsockopt(subscriber, ZMQ_SUBSCRIBE, "AmongUs>kick4?>", 15);
        memset(buffer,0,256);
        zmq_recv(subscriber, buffer, 256,0);
        ParsedString = parse(3, buffer);
        printf("%s\n", ParsedString);
    ...
    zmq_setsockopt(subscriber, ZMQ_SUBSCRIBE, "AmongUs>kick4?>", 15);
    ...
    e: if((strcmp(ParsedString, "You have been kicked\n")) == 0)
    {
        ...
        zmq_setsockopt(subscriber, ZMQ_UNSUBSCRIBE, "AmongUs>kick4?>", 16 );
