...
{
    //bericht
    const char *BerichtPlayer = (argc > 1)? argv [1]: "AmongUs>player5!>";
...
while(1)
{
    ...
    zmq_setsockopt (subscriber, ZMQ_SUBSCRIBE, "AmongUs>imposter5?>", 19);
    zmq_setsockopt (subscriber, ZMQ_SUBSCRIBE, "AmongUs>crewmember5?>", 21);
    memset(buffer,0,256);
    zmq_recv(subscriber, buffer, 256,0);
    ParsedString = parse(3, buffer);
    printf("%s\n", ParsedString);
    ...
    zmq_setsockopt(subscriber, ZMQ_UNSUBSCRIBE, "AmongUs>imposter5?>", 19);
    zmq_setsockopt(subscriber, ZMQ_UNSUBSCRIBE,"AmongUs>crewmember5?>", 21);
    for (int i = 0 ;i<players ;i++)
    {
        ...
        k: zmq_setsockopt(subscriber, ZMQ_SUBSCRIBE, "AmongUs>kick5?>", 15);
        memset(buffer,0,256);
        zmq_recv(subscriber, buffer, 256,0);
        ParsedString = parse(3, buffer);
        printf("%s\n", ParsedString);
   ...
   s: printf("waiting for votes to be counted...\n\n");
    ...
    zmq_setsockopt(subscriber, ZMQ_SUBSCRIBE, "AmongUs>kick5?>", 15);
    ...
    e: if((strcmp(ParsedString, "You have been kicked\n")) == 0)
    {
        ...
        zmq_setsockopt(subscriber, ZMQ_UNSUBSCRIBE, "AmongUs>kick5?>", 15 );
